...
const HeroSlider = ({
    ...
    // Loop manually when loading finishes
    useEffect(() => {
        if (swiperInstance && !isLoading) {
            swiperInstance.autoplay.start();
        } else if (swiperInstance && isLoading) {
    ...
    return (
        <hero-slider className={`hero-slider ${wrapperCls} ${!isLoading ? 'animate-ready' : ''}`}>
            <Swiper
                slidesPerView={1}
                loop={true}
